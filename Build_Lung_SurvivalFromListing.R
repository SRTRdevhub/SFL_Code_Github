###
### This function will build the survival from listing model for lung candidates
###
...
follow_up_cuts <- c(0, 183, 2 * 365, 3 * 365 + 183)
...
peds_0_12_mi_covariates <- control$var_name[control$ped_0_12_build_include]
peds_0_12_mi_cov_method <- control$MI_TYPE[control$ped_0_12_build_include]

peds_12_18_mi_covariates <- control$var_name[control$ped_12_18_build_include]
peds_12_18_mi_cov_method <- control$MI_TYPE[control$ped_12_18_build_include]
...
if(! file.exists(file.path(root, organ, "MI_Output", paste0("mi_", organ_ab, "_adult_data_", mi_num, ".Rdata")))){
  ...
  sfl_data <- LungDataCleaning(root, 
                               organ, 
                               organ_ab, 
                               current_saf, 
                               cohort_start, 
  ...
  trim_vars <- control$var_name[{control$adult_build_include | control$ped_0_12_build_include | control$ped_12_18_build_include} & {control$MI_TYPE == "continuous"}]
  ...
  for(k in trim_vars){
    ...
    peds_0_12_boo  <- {sfl_data$can_age < 12}
    peds_12_18_boo <- {sfl_data$can_age >= 12} & {sfl_data$can_age < 18}
    ...
    peds_0_12_q1  <- quantile(sfl_data[[k]][peds_0_12_boo], 
                              probs = 1/100,
                              na.rm = TRUE)
    peds_0_12_q99 <- quantile(sfl_data[[k]][peds_0_12_boo], 
                              probs = 99/100,
                              na.rm = TRUE)
    
    
    peds_12_18_q1  <- quantile(sfl_data[[k]][peds_12_18_boo], 
                               probs = 1/100,
                               na.rm = TRUE)
    peds_12_18_q99 <- quantile(sfl_data[[k]][peds_12_18_boo], 
                               probs = 99/100,
                               na.rm = TRUE)
    ...
    sfl_data[[k]][peds_0_12_boo] <- ifelse(sfl_data[[k]][peds_0_12_boo] < peds_0_12_q1,
                                           peds_0_12_q1,
                                           ifelse(sfl_data[[k]][peds_0_12_boo] > peds_0_12_q99,
                                                  peds_0_12_q99,
                                                  sfl_data[[k]][peds_0_12_boo]))
    
    
    sfl_data[[k]][peds_12_18_boo] <- ifelse(sfl_data[[k]][peds_12_18_boo] < peds_12_18_q1,
                                            peds_12_18_q1,
                                            ifelse(sfl_data[[k]][peds_12_18_boo] > peds_12_18_q99,
                                                   peds_12_18_q99,
                                                   sfl_data[[k]][peds_12_18_boo]))
  }
  
  
  sfl_data_peds_0_12 <- sfl_data %>%
    subset(can_age < 12)
  
  
  sfl_data_peds_12_18 <- sfl_data %>%
    subset({can_age >= 12} & {can_age < 18})
  ...
  rm(list = c("sfl_data", "adult_boo", "peds_0_12_boo", "peds_12_18_boo"))
  gc()
  
  
  ### Saving the raw datasets for (1) identifying missingness for the LBV adjustment, and (2) the appropriate reference level
  save(sfl_data_peds_0_12,
       file = file.path(root,
                        organ,
                        "sfl_data_peds_0_12.Rdata"))
  
  
  save(sfl_data_peds_12_18,
       file = file.path(root,
                        organ,
                        "sfl_data_peds_12_18.Rdata"))
  ...
  mi(sfl_data_peds_0_12,
     mi_num,
     peds_0_12_mi_covariates,
     peds_0_12_mi_cov_method,
     keep_vars,
     "peds_0_12",
     file.path(root, organ, "MI_Output"))
  
  
  mi(sfl_data_peds_12_18,
     mi_num,
     peds_12_18_mi_covariates,
     peds_12_18_mi_cov_method,
     keep_vars,
     "peds_12_18",
     file.path(root, organ, "MI_Output"))
  ...
  rm(list = c("sfl_data_adult", "sfl_data_peds_0_12", "sfl_data_peds_12_18"))
  gc()
}
