###
### This function will build the survival from listing model for heart candidates
###
...
if(! file.exists(file.path(root, organ, "MI_Output", paste0("mi_", organ_ab, "_adult_data_", mi_num, ".Rdata")))){
  ...
  sfl_data <- HeartDataCleaning(root, 
                                organ, 
                                organ_ab, 
                                current_saf, 
                                cohort_start, 
  ...
  for(k in trim_vars){
    ...
    adult_boo <- {sfl_data$can_age >= 18} & {! is.na(sfl_data$can_age)}
    peds_boo  <- {sfl_data$can_age < 18} | is.na(sfl_data$can_age)
