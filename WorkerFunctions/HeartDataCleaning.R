...
HeartDataCleaning <- function(root, 
                              organ, 
                              organ_ab, 
                              current_saf, 
                              cohort_start, 
  ...
  ### Extracting the numbers from the candidate diagnosis label
  cand$can_dgn_num <- sapply(cand$CAN_DGN, function(x){ strsplit(x, split = ":")[[1]][1] })
  ...
  cand <- cand %>%
    ...
    mutate(can_male = ifelse(CAN_GENDER == "", NA,
           ...
           can_lf_support = ifelse(CAN_LIFE_SUPPORT %in% c("", NA),
                                   NA,
                                   ifelse(CAN_LIFE_SUPPORT == "Y", 1, 0)),
           can_vad_status = ifelse(CAN_VAD_TY %in% c("", NA), 
                                   NA,
                                   ifelse(CAN_VAD_TY == "1: NONE", 0, 1)),
                                   ...
                                   ifelse(CAN_ABO %in% c("A: A", "A1: A1", "A2: A2"), "A",
                                          ifelse(CAN_ABO %in% c("AB: AB", "A1B: A1B", "A2B: A2B"), "AB",
                                                 ifelse(CAN_ABO %in% c("B: B"), "B",
                                                        ifelse(CAN_ABO %in% c("O: O"), "O", 
                                                               ifelse(CAN_ABO %in% c("Z: Z (In Utero Only)"),
                                                                      "Z",
                                                                      "Unexpected")))))),
           can_blood_type_missing = ifelse(CAN_ABO %in% c("", "UNK"), 1, 0),
           can_dgn_prim = ifelse(is.na(can_dgn_num), NA,
                                 ifelse(can_dgn_num %in% c("1000", "1001", "1002", "1003", "1004", "1005", "1006", "1049", 
                                                           "1050", "1051", "1052", "1053", "1054", "1099", "1201"),
                                        "Cardiomyopathy",
                                        ifelse(can_dgn_num %in% c("1100", "1101", "1102", "1103", "1104", "1105", "1106", "1199"),
                                               "RetransplantGraftFailure",
                                               ifelse(can_dgn_num %in% c("1007", "1200"), 
                                                      "CoronaryArteryDisease",
                                                      ifelse(can_dgn_num %in% "1202", 
                                                             "ValvularHeartDisease",
                                                             ifelse(can_dgn_num %in% c("1203", "1205", "1206", "1207"),
           ...
           can_dialysis_status = ifelse(CAN_DIAL %in% c(NA, "", "998: Dialysis Status Unknown"), 
                                        NA,
                                        ifelse(CAN_DIAL %in% c("1: No dialysis"), 0, 1)),
           can_symp_cvd = ifelse(CAN_CEREB_VASC %in% c(NA, "", "U"),
                                 NA,
                                 ifelse(CAN_CEREB_VASC == "Y", 1, 0)),
           can_prior_cancer = ifelse(CAN_MALIG %in% c("", "U"), NA,
                                     ifelse(CAN_MALIG == "Y", 1, 0)),
           can_prior_cancer_missing = ifelse(CAN_MALIG %in% c("", "U"), 1, 0),
           can_s_cr = ifelse(is.na(CAN_MOST_RECENT_CREAT),
                             NA,
                             CAN_MOST_RECENT_CREAT),
           can_defibrillator = ifelse(CAN_IMPLANT_DEFIB %in% c(NA, "", "U"),
                                      NA,
                                      ifelse(CAN_IMPLANT_DEFIB == "Y", 1, 0)),
           ...
           can_hemo_co = CAN_CARDIAC_OUTPUT,
           ...
           can_prior_congen_surgery = ifelse(CAN_PRIOR_CONGEN_SURG %in% c("", "U", NA), NA,
                                   ...
                                   ifelse(CAN_ACADEMIC_LEVEL == "996: Not Applicable < 5 years old/ High School graduate or GED",
                                          ...
                                          ifelse(CAN_ACADEMIC_LEVEL == "1: Full academic load",
                                                 ...
                                                 ifelse(CAN_ACADEMIC_LEVEL == "2: Reduced academic load",
                                                        "ReducedLoad",
                                                        "CannotParticipate")))),
           can_hf_loss = ifelse(CAN_SUDDEN_DEATH %in% c(NA, "", "U"),
                                NA,
                                ifelse(CAN_SUDDEN_DEATH == "Y", 1, 0)),
  ...
  ### Importing the stathist file for allocation status at listing
  
  stathist <- read.csv(file.path(root,
                                 "Data Sets",
                                 paste0(organ_ab, 
                                        "_stathist_", 
                                        current_saf,
  ...
  ### Selecting the appropriate row in the stathist file
  cand_stathist <- cand_stathist %>%
    subset(as.Date(CAN_LISTING_DT, format = "%m/%d/%y") == as.Date(CANHX_BEGIN_DT, format = "%m/%d/%Y")) %>%
    mutate(status_1a_period = 1 * {CANHX_STAT_CD %in% "2010: Status 1A"},
           status_1b_period = 1 * {CANHX_STAT_CD %in% "2020: Status 1B"},
           status_2_period  = 1 * {CANHX_STAT_CD %in% "2030: Status 2"},
           can_status = ifelse(status_1a_period == 1, 1,
                               ifelse(status_1b_period == 1, 2,
                                      ifelse(status_2_period == 1, 3, NA))))
  
  
  ### Checking that the selection of appropriate rows from the stathist file worked
  if(nrow(cand_stathist) != nrow(cand)){ stop("The stathist cleaning failed.") }
  
  
  cand <- cand_stathist
  
  
  rm(list = c("stathist", "cand_stathist"))
  gc()
