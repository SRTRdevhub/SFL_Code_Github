###
### This function will build the survival from listing model for liver candidates
###
...
follow_up_cuts <- c(0, 91, 182, 365 * 1:{max_followup - 1})
...
if(! file.exists(file.path(root, organ, "MI_Output", paste0("mi_", organ_ab, "_adult_data_", mi_num, ".Rdata")))){
  ...
  sfl_data <- LiverDataCleaning(root, 
                                organ, 
                                organ_ab, 
                                current_saf, 
                                cohort_start, 
...
if(! file.exists(file.path(root, organ, "piecewise_separate", paste0("adult_cv_prob_", 10, ".csv")))){
